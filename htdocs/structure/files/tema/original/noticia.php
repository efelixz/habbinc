...
while ($row2 = $query1->fetch_assoc()) {
    ...
    $authorData = mysqli_fetch_assoc($query12);
    ?>

    <div class="article-author-image rounded-circle shadow-sm" 
         style="width: 70px; height: 70px; background-image: url('https://habbo.city/habbo-imaging/avatarimage?figure=<?php echo $authorData['look']; ?>&size=m&head_direction=2&gesture=sml'); background-size: cover; background-position: center;">
    </div>

    <div class="article-author" style="font-size: 14px; color: #333;">
        <p class="mb-1"><strong>Autor:</strong> 
            <a href="/perfil?=<?php echo $row3['author']; ?>" 
               style="text-decoration: none; color: #007bff;"
               onmouseover="this.style.textDecoration='underline';" 
               onmouseout="this.style.textDecoration='none';">
               <?php echo htmlspecialchars($row3['author']); ?>
            </a>
        </p>
        <p class="mb-1">
        ...
        <p class="mb-0">
            <small class="text-muted">
                ...
                Visualizações: <?php echo $row3['type']; ?>
            </small>
        </p>
    </div>
