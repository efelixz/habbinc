...
$cargoImagens = [
    10 => ['nome' => 'Guardiões', 'imagem' => 'https://i.imgur.com/XYxyAbC.png'], // troque a imagem se quiser
    9 => ['nome' => 'AeA/GeA', 'imagem' => 'https://i.imgur.com/ABcdEfG.png'], // troque a imagem se quiser
];
...
function exibirUsuarioCompacto($user, $avatarimage, $cargoImagens, $isOnline) {
    ...
    $cargo = isset($cargoImagens[$rank]) ? $cargoImagens[$rank]['nome'] : 'Colaborador';
...
function listarUsuariosPorRank($conn, $rank, $cargoImagens, $avatarimage) {
    ...
    $info = $cargoImagens[$rank] ?? ['nome' => 'Colaborador', 'imagem' => ''];
...
<div class="container mt-4">
    ...
    <h2 class="equipes-titulo">Colaboradores</h2>
    ...
    <div class="tabs-wrapper">
        ...
        <div class="tab-content" id="tab-colaboradores" style="display: block;">
            <?php
...
document.addEventListener('DOMContentLoaded', () => {
    // Tab switching for Colaboradores only (Staff and Rádio are links)
    document.querySelectorAll('.tab-btn').forEach(btn => {
        btn.addEventListener('click', e => {
            if (btn.dataset.tab === 'colaboradores') {
                document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
                document.querySelectorAll('.tab-content').forEach(c => c.style.display = 'none');
                document.getElementById('tab-colaboradores').style.display = 'block';
            }
        });
    });
});
